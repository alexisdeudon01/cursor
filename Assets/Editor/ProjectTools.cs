...
public static class ProjectTools
{
    ...
    #region Direct Git Push Buttons

    [MenuItem("Tools/Push to branché-1 (avec vérification)", false, 100)]
    public static void PushToBranche1WithVerification()
    {
        if (!VerifyGitStatus())
        {
            return;
        ...
        if (!EditorUtility.DisplayDialog("Confirmation",
            "Voulez-vous pousser vers branché-1?\n\n" +
            "Le script va vérifier l'état Git et demander confirmation.",
            "Oui", "Non"))
        {
            return;
        }

        ExecuteGitScriptInteractive("push_to_branche1.sh", "Push vers branché-1");
    }

    [MenuItem("Tools/Push to dev", false, 101)]
    public static void PushToDevDirect()
    {
        if (!VerifyGitStatus())
        {
            return;
        ...
        ExecuteGitScriptInteractive("push_to_dev.sh", "Push vers dev");
    }

    private static bool VerifyGitStatus()
    {
        ...
        {
            EditorUtility.DisplayDialog("Erreur", "Ce projet n'est pas un dépôt Git!", "OK");
            return false;
        }
    ...
    private static void ExecuteGitScriptInteractive(string scriptName, string actionName)
    {
        string projectPath = Path.GetDirectoryName(Application.dataPath);
        string scriptPath = Path.Combine(projectPath, scriptName);

        if (!File.Exists(scriptPath))
        {
            EditorUtility.DisplayDialog("Erreur",
                $"Script non trouvé: {scriptName}\n\n" +
                $"Chemin attendu: {scriptPath}",
                "OK");
            return;
        }
        ...
        // Exécuter le script dans un terminal interactif
        ...
        {
            Process.Start(psi);
            UnityEngine.Debug.Log($"[ProjectTools] {actionName} - Script lancé dans le terminal");
        }
        catch (System.Exception ex)
        {
            UnityEngine.Debug.LogError($"[ProjectTools] Erreur: {ex.Message}");
            EditorUtility.DisplayDialog("Erreur",
